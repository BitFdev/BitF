...
#if defined(NDEBUG)
#error "BitF cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uBitF) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight > 21 && nHeight <= 9999999)
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BitF: We use certain sporks during IBD, so check to see if they are
